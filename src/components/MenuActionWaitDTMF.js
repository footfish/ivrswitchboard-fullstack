...
const menuOptions = [ { value: "0", label: "0"}, 
    ...
    return(
        ...
        return(
        <button key={menuOption.value} value={!buttonActive} name={menuOption.value} type="button" className={buttonActive ? 'ml-1 btn btn-sm btn-primary' : 'ml-1 btn btn-sm btn-outline-secondary'    } onClick={handleClick} >{menuOption.label}</button>
        )})}
        </div></div>
    )
}
